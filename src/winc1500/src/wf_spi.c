...
void nm_spi_init(void)
{
    ...
    {
        dprintf("Invalid chip rev, expected %lx, read %lx\n", (long unsigned int)SUPPORTED_WINC1500_CHIP_REVISION, (long unsigned int)chipId);
...
_FAIL_:
    if(result != N_OK)
    {
        DelayMs(1);
        nm_spi_reset();
        dprintf("Reset and retry %d %lx %lx\n",retry, (long unsigned int)addr, (long unsigned int)data);
        DelayMs(10);
        retry--;
...
_FAIL_:
    if(result != N_OK)
    {
        DelayMs(1);
        nm_spi_reset();
        dprintf("Reset and retry %d %lx, %x\n",retry, (long unsigned int)addr, size);
        DelayMs(1);
        retry--;
        if(retry) goto _RETRY_;
    }

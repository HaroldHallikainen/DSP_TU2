...
double AdcSamplef, LimiterIn, LimiterOut, MarkSample, SpaceSample, MarkDemodOut, SpaceDemodOut, DiscrimOut, DdsOut;  // These were originally in main but seemed to get corrupted
...
#define UsePreLimiterBpf       // Define to include a BPF in front of limiter
  ...
  double PreLimiterBpfQ = 10;           // Q=10 did not appear to add bias distortion. 20 adds severe bias distortion.
#endif
    ...
    while ( true ){
      if(Timer2TimeoutCounter==0){        // We have timed out 10 times, so it has been 125 us
        ...
        switch(AudioOut){       // Provide analog output for debug
            ...
            case dds:           AudioPwmSet(DdsOut); break;
        }

...
enum {NONE,ADC, AGC, INPUT_BPF, LIMITER, MARK_FILTER_OUT, SPACE_FILTER_OUT, MARK_DEMOD_OUT, SPACE_DEMOD_OUT, DISCRIM, DDS, THRESHOLD, DISCRIM_LESS_THRESHOLD} AudioOut=MARK_FILTER_OUT;
...
int main ( void ){
    ...
    while ( true ){
      if(Timer2TimeoutCounter<1){        // We have timed out 10 times, so it has been 125 us
        ...
        // DisplayWriteNextPixel(0x00, 0xff, 0x00);    // display test
